...
export async function login(req: Request): Promise<Response> {
  try {
    ...
    const message = `
    User : 65130496@dpu.ac.th logged in
    token: ${token}`;
    SendToLine(message);
...
export async function register(req: Request): Promise<Response> {
  try {
    ...
    // ���ร้างข้อมูลผู้ใช้ 
    let nameTH = "ศุภิสรา ชีวนันทพร (จินนี่)";
    let nameEN = "Suphitsara Cheevanantaporn (Jinny)";
    let studentID = "65130496";
    username = studentID;
